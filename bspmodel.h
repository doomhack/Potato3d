...
namespace P3D
{
    ...
    {
    public:
        Stack(unsigned int size = 8192)  { pos = mem = (T*)new char[sizeof(T) * size]; this->size = size; }
        ~Stack()                        { delete[] mem; }
    ...
    {
    public:
        List(unsigned int size = 8192) { pos = mem = (T*)new char[sizeof(T) * size]; this->size = size; }
        ~List()                        { delete[] mem; }
